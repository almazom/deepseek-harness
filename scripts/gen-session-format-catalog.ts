...
export function renderSessionFormatCatalog(
  ...
  const arrayField = (name: string, values: readonly string[]): string => {
    const inline = `  ${name}: [${values.join(', ')}],`
    if (values.length === 0 || inline.length <= 140) return inline
  ...
  return [
    ...
    arrayField('codecs', codecs),
    `  currentEncoder: ${currentCodec},`,
    arrayField('migrations', declarations.map(item => item.migration)),
