...
/** A stable wide-frame narrow fact for constructions that never read it. */
function narrowFact(): ObservableSnapshot<boolean> {
  return createSnapshotStore<boolean>(false)
}

describe('LayoutController', () => {
  it('exposes the constructor-supplied narrow fact by identity', () => {
    const narrow = narrowFact()
    const service = new LayoutController(fakePanels(), () => true, narrow)
    expect(service.narrow).toBe(narrow)
  })

  it('forwards right column transitions to the constructor-supplied actions', () => {
    const panels = fakePanels()
    const service = new LayoutController(panels, () => true, narrowFact())
  ...
  it('keeps separately constructed controllers bound to their own instances', () => {
    ...
    const firstService = new LayoutController(first, () => true, narrowFact())
    const secondService = new LayoutController(second, () => true, narrowFact())
  ...
  it('rejects an absent main entry without changing selection or cancelling pending navigation', () => {
    ...
    const service = new LayoutController(panels, id => present.has(id), narrowFact())
  ...
  it('supersedes asynchronous navigation on another request, any valid selection, and disposal', () => {
    const service = new LayoutController(fakePanels(), () => true, narrowFact())
