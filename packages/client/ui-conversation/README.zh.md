...
队列坞的 Smart-steer 入口（行按钮、待处理占位符与顾问界面插槽）仅在 `smartSteer` 配置不为 `false` 时渲染；该开关只管展示，因此希望彻底关闭该功能的部署还需在 cordis patch 中禁用 Smart-steer 插件行与 `queue-advisor` host 路由，此后顾问请求将以 `session/advisor-unavailable` 响亮失败。
