...
The queue dock's Smart-steer entry points (row button, pending placeholder, and the advisor surface slot) render only while the `smartSteer` config is not `false`; the flag is presentation-only, so a deployment that wants the feature fully offline also disables the Smart-steer plugin rows and the `queue-advisor` host route in its cordis patch, after which an advisory request fails loud with `session/advisor-unavailable`.
