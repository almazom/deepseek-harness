/**
 * Session-browsing core behind the sidebar region's chrome: the remote-search
 * and reveal state, blank-session promotion, order-account sync, the search
 * results / grouped tree / flat list bodies, and the workspace and session
 * dialogs. WorkspaceBrowser owns the sidebar chrome and the search-query
 * state and mounts this core inside its list area; a future full-page
 * surface can mount the same core.
 */
...
import type {
  WorkspaceId, WorkspaceListPhase, WorkspaceSnapshot, WorkspaceView,
...
> & {
  ...
  /** Open the core-owned rename dialog for a real Workspace group. */
  onRenameRequest: (workspaceId: WorkspaceId, currentTitle: string) => void
  /** Open the core-owned delete-confirmation dialog for a real Workspace group. */
  onDeleteRequest: (workspaceId: WorkspaceId, currentTitle: string) => void
  /** Open the core-owned session rename dialog. */
...
type SessionBrowserCoreProps =
  ...
  & {
    ...
    /** Collapse the search (a search-result selection leaves it). */
    onSearchExpandedChange: (expanded: boolean) => void
    /** Host account home for POSIX hover-path abbreviation. */
    home?: string | undefined
    workspaces: readonly WorkspaceView[]
    /** Workspace-list arrival phase gating blank promotion and account retention. */
    workspacePhase: WorkspaceListPhase
    /** Workspace stream state completing the readiness check with the phase. */
    workspaceStreamState: WorkspaceSnapshot['state']
    /** Registry-global archive set (hidden rows). */
    archivedSessionIds: readonly SessionNode['id'][]
...
/**
 * Render the browsing core: the wide list bodies, the always-mounted
 * workspace/session dialogs, and the core effects (blank promotion, account
 * retention, remote search, reveal bookkeeping).
 * @param props - core inputs plus the shared query/search callbacks.
 * @returns the core element tree.
 */
export function SessionBrowserCore({
  ...
  // Rename dialog (core-owned so it outlives row unmounts during collapse).
  ...
  // Session rename dialog (same core-owned pattern as workspace rename;
