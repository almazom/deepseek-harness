...
与设置与命令包一起挂载本插件；权限行随即出现在通用设置中，`/permission` 选择器替换裸命令调用。当前会话选择器在会话的投影 key 存在、或宿主提供 permission 设置命名空间时可用；无权限组合既不显示选择器，也不显示设置行。

### 选择器

在尚未物化会话的界面上（例如新建会话屏幕），选择器改为读取 permission 设置命名空间的预设：当前默认值标记为激活，选择某一项会通过通用设置行所用的同一设置变更把它写为新会话默认值；当前会话不受影响。
