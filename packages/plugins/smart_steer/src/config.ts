 ...
 * @module @deepseek-ai/dsh-smart-steer/config
 */
...
function assertPositiveInteger(name: string, value: number): void {
  if (!Number.isInteger(value) || value <= 0) {
    throw new Error(`smart_steer: ${name} must be a positive integer`)
  }
}
...
export function resolveAdvisorLlmConfig(config: AdvisorLlmConfig): ResolvedAdvisorLlmConfig {
  ...
  if (candidate === null || typeof candidate !== 'object') {
    throw new Error('smart_steer: configuration is required')
  }
  const value = candidate as AdvisorLlmConfig
  for (const key of Object.keys(value)) {
    if (!CONFIG_KEYS.has(key)) throw new Error(`smart_steer: unknown config key "${key}"`)
  }
  ...
  if (value.timeoutMs > MAX_TIMER_DELAY_MS) {
    throw new Error(`smart_steer: timeoutMs must not exceed ${MAX_TIMER_DELAY_MS}`)
  }
  if ((value.provider === undefined) !== (value.model === undefined)) {
    throw new Error('smart_steer: provider and model must be paired')
  }
...
export function buildAdvisorMessages(input: AdvisorPromptInput): readonly Message[] {
  ...
  if (byteLength(queued) > input.maxInputBytes) {
    throw new Error('smart_steer: queued message exceeds maxInputBytes')
  }
  ...
  for (;;) {
    ...
    if (tail.length === 0) throw new Error('smart_steer: snapshot exceeds maxInputBytes')
    tail.shift()
  }
  return [createUserMessage({
    content: [{ type: 'text', text: framed }],
    source: { kind: 'plugin', plugin: 'smart_steer' },
  })]
}
