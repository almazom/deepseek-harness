 ...
 * @module @deepseek-ai/dsh-smart-steer/dispatcher
 */
...
import { messageText } from './latest-human.ts'
import { MessageId } from '@deepseek-ai/dsh-llm/brand'
import { deadline } from '@deepseek-ai/dsh-timeout'
import { deepFreeze } from '@deepseek-ai/dsh-util-values'
import { SessionSeq, type Session, type SessionEvent } from '@deepseek-ai/dsh-session'
import type {} from '@deepseek-ai/dsh-session-query'
import { AdvisorSectionWatcher, ADVISOR_RUN_TIMEOUT_CODE, AdvisorLlmConfigFields, buildAdvisorMessages, buildAdvisorSystemPrompt, resolveAdvisorLlmConfig } from './config.ts'
import { advisorRunProjectionDefinition } from './advisor-projection.ts'
...
export default class QueueAdvisorService extends Service {
  ...
  constructor(ctx: Context, config: AdvisorLlmConfig) {
    ...
    if (candidate.provider === undefined || candidate.model === undefined) {
      throw new Error('smart_steer: the dispatcher requires an explicit provider/model pair')
    }
