...
async function loadAdvisorYaml(lines: readonly string[]): Promise<Context> {
  ...
  const modules = new Map<string, unknown>([
    ...
    ['@deepseek-ai/dsh-smart-steer/dispatcher', AdvisorDispatcher],
    ['@deepseek-ai/dsh-smart-steer', RootPlugin],
  ])
...
describe('real Loader composition', () => {
  it('boots the advisory YAML shape and lands one whole advisory side run', async () => {
    const loaded = await loadAdvisorYaml([
      ...
      "- name: '@deepseek-ai/dsh-smart-steer/dispatcher'",
      '  config:',
      '    maxInputBytes: 4000',
      '    maxOutputTokens: 256',
      '    timeoutMs: 2000',
  ...
  it('unregisters the advisor projection unit and service slot on disposal', async () => {
    ...
    const advisorEntry = [...loaded.loader.entries()]
      .find(entry => entry.options.name === '@deepseek-ai/dsh-smart-steer/dispatcher')
  ...
  it('mounts the root plugin with a paired route and constructs the queue-advisor service', async () => {
    const loaded = await loadAdvisorYaml([
      "- name: '@deepseek-ai/dsh-session'",
      "- name: '@deepseek-ai/dsh-llm'",
      "- name: '@deepseek-ai/dsh-session-projection'",
      "- name: '@deepseek-ai/dsh-session-query'",
      "- name: '@deepseek-ai/dsh-smart-steer'",
      '  config:',
      '    maxInputBytes: 4000',
      '    maxOutputTokens: 256',
      '    timeoutMs: 2000',
    ...
    expect(unloaded).toEqual([])
    expect(loaded.queueAdvisor).toBeDefined()
    const session = loaded.sessions.create(SessionId('root-composed'))
    session.append('user/message', createUserMessage({
      content: [{ type: 'text', text: 'ответь на это' }],
      source: { kind: 'user' },
    }), { surfaceOp: 'append' })
    // The root mount registers both projection units; the advisor/run unit is
    // present the moment its owning service exists.
    expect(loaded.sessionProjections.snapshot(session)
      .values['advisor/run']).toBeDefined()
  })

  it('mounts the root plugin config-less or half-paired without the dispatcher service', async () => {
    const rows: readonly string[][] = [
      // Preset shape: no config row at all; only the command surface mounts.
      [
        "- name: '@deepseek-ai/dsh-session'",
        "- name: '@deepseek-ai/dsh-session-projection'",
        "- name: '@deepseek-ai/dsh-smart-steer'",
      ],
      // Limits without a route stay command-only.
      [
        "- name: '@deepseek-ai/dsh-session'",
        "- name: '@deepseek-ai/dsh-session-projection'",
        "- name: '@deepseek-ai/dsh-smart-steer'",
      ...
      // A provider without its model is not a route.
      [
        "- name: '@deepseek-ai/dsh-session'",
        "- name: '@deepseek-ai/dsh-session-projection'",
        "- name: '@deepseek-ai/dsh-smart-steer'",
    ...
    for (const lines of rows) {
      const loaded = await loadAdvisorYaml(lines)
      const unloaded = [...loaded.loader.entries()]
        .filter(entry => entry.fiber === undefined && !entry.disabled)
        .map(entry => entry.options.name)
      expect(unloaded).toEqual([])
