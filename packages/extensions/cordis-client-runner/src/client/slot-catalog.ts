...
export const CLIENT_SLOT_API: readonly ClientSlotEntry[] = [
  ...
  {
    key: 'conversation.input.dock.advisor',
    kind: 'single',
    scope: 'session',
    summary: 'The advisor surface anchored to the input dock: a `single` session slot the smart_steer plugin registers into.',
    doc: 'The advisor surface anchored to the input dock: a `single` session slot\nthe smart_steer plugin registers into. Absent without it: the queue dock\nrenders no advisor and its smart button still opens the run for the\nprojected answer, but no sheet appears to read or act on it.',
    registerOptions: [],
    ownerProps: [
    ...
    hookContext: '',
    slotInject: '',
    declaredBy: 'an entry in \'conversation.input.dock\' (client-ui-conversation), so it exists while that entry is mounted',
    occupants: [
      'smart-steer AdvisorSurface',
    ],
    replaceRisk: 'shadows-shipped-ui',
    example: 'return {\n  inject: [\'slots\'],\n  apply(ctx) {\n    ctx.slots.inject(\'conversation.input.dock.advisor\', () => ctx.slots.register(\n      { name: \'conversation.input.dock.advisor\' },\n      () => React.createElement(\'div\', null, \'hello\'),\n    ))\n  },\n}',
    source: 'packages/plugins/smart_steer/src/client/owner.ts:62',
  },
  {
